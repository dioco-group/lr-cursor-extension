...
class PreviewPanel {
    _extensionUri;
    static viewType = 'lr-course-editor.preview';
    static _panels = new Map();
    _panel;
    _document;
    _disposables = [];
    static createOrShow(document, extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel(PreviewPanel.viewType, `Preview: ${getFileName(document.uri)}`, { viewColumn: vscode.ViewColumn.Beside, preserveFocus: true }, { enableScripts: true, localResourceRoots: [extensionUri] });
        const preview = new PreviewPanel(panel, document, extensionUri);
        PreviewPanel._panels.set(key, preview);
    }
    constructor(panel, document, _extensionUri) {
...
function renderVoiceConfig(m) {
    ...
    const entries = [];
    if (vc.default) {
        entries.push(renderVoiceEntry('Default', vc.default));
    }
    if (vc.prompt) {
        entries.push(renderVoiceEntry('Prompt', vc.prompt));
    }
    if (vc.response) {
        entries.push(renderVoiceEntry('Response', vc.response));
    }
    if (vc.introVoice) {
        entries.push(renderVoiceEntry('Intro', vc.introVoice));
    }
...
function renderVoiceEntry(label, spec) {
    if (!spec) {
        return '';
    }
